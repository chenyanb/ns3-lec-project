...
//创建wifista无线终端，AP接入点
  ...
  wifiStaNodes.Create (nWifi);////3个WIFI再加上一个P2P节点（n0）
...
//配置AP移动方式，ConstantPositionMobilityModel，固定位置模型
 ...
 //wifi信道
  ...
  ApplicationContainer serverApps = echoServer.Install (csmaNodes.Get (nCsma));////服务程序安装到CSMA的最后一个设备n4
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (10.0));/////////////设置应用程序开始以及结束时间，1 秒开始10秒结束

  //回显客户端放在最后的STA节点，指向CSMA网络的服务器，上面的节点地址，端口为9
  UdpEchoClientHelper echoClient (csmaInterfaces.GetAddress (nCsma), 9);/////////安装客户端应用程序，，客户端发送的目标节点IP，，即客户端程序往哪里发送数据，;
//发送到n4,n4为服务器端程序安装的节点，端口为9
  ...
  ApplicationContainer clientApps = 
    echoClient.Install (wifiStaNodes.Get (nWifi - 1));//////安装了一个应用程序
