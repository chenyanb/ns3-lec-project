...
main (int argc, char *argv[])
{
 ...
 NodeContainer  csmaApNode=p2pNodes.Get (0);
 ...
 //csma信道
  ...
  ApplicationContainer serverApps = echoServer.Install (csmaNodes.Get (ncsma-2));
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (10.0));/////////////1 秒开始10秒结束

  //回显客户端放在最后的STA节点，指向CSMA网络的服务器，上面的节点地址，端口为9
  UdpEchoClientHelper echoClient (csmaInterfaces.GetAddress (ncsma-2),19);
 ...
 { echoClient.Install (csmaNodes.Get (ncsma - 1)),echoClient.Install (csmaNodes.Get (ncsma - 3)),echoClient.Install (csmaNodes.Get (ncsma - 4)),
echoClient.Install (csmaNodes.Get (ncsma - 5))};
for(int i=0;i<4;i++)
 { 
